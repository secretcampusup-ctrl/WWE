...
/// Temporary clean baseline that uses Apple's standard player only.
/// The cinematic player is intentionally not instantiated, so none of its
/// custom buffering, thumbnail, network-reset, MKV, or overlay logic runs.
private struct NativeSystemPlayerView: View {
    @Environment(\.dismiss) private var dismiss
    let url: URL
    let title: String
    let resumeAt: Double
    ...
    @StateObject private var model: NativeSystemPlayerModel
    ...
    init(
        ...
        onProgress: ((Double, Double, Int, Int) -> Void)?
    ) {
        self.url = url
        self.title = title
        ...
        _model = StateObject(wrappedValue: NativeSystemPlayerModel(
            url: url,
            resumeAt: resumeAt,
            httpHeaders: httpHeaders,
            onProgress: onProgress
    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
            Color.black.ignoresSafeArea()
            NativePlayerController(player: model.player).ignoresSafeArea()
            Button {
                model.stop()
                dismiss()
            } label: {
...
@MainActor
private final class NativeSystemPlayerModel: ObservableObject {
    let player: AVPlayer
    private let onProgress: ((Double, Double, Int, Int) -> Void)?
    ...
    init(
        ...
        timeObserver = player.addPeriodicTimeObserver(
            ...
            self.onProgress?(time.seconds, duration, Int(size.width), Int(size.height))
        }
    }
    ...
    func stop() {
        ...
        if let item = player.currentItem {
            let duration = item.duration.seconds
            let size = item.presentationSize
            if time.isFinite, duration.isFinite, duration > 0 {
                onProgress?(time, duration, Int(size.width), Int(size.height))
            }
        }
        player.pause()
