...
enum MilkieClient {
    ...
    static func search(
        ...
        guard (200..<300).contains(http.statusCode) else { throw MilkieError.http(http.statusCode) }

        let payload = try JSONDecoder().decode(MilkieSearchPayload.self, from: data)
        return payload.torrents.map(\.model)
    }
    ...
    static func details(id: String) async throws -> MilkieTorrent {
        ...
        let payload = try JSONDecoder().decode(MilkieDetailsPayload.self, from: data)
...
private struct MilkieSearchPayload: Decodable {
    let torrents: [MilkieRow]
}

private struct MilkieDetailsPayload: Decodable {
    let torrent: MilkieRow
}

private struct MilkieRow: Decodable {
    let id: String
    let releaseName: String
    let category: Int?
    let size: MilkieFlexibleNumber
    let seeders: MilkieFlexibleNumber?
    let leechers: MilkieFlexibleNumber?
    ...
    var model: MilkieTorrent {
        ...
        return MilkieTorrent(
            ...
            infoHash: (hash?.isEmpty == false) ? hash : nil,
...
private struct MilkieExternals: Decodable {
    let imdb: String?
    let tmdb: Int?
}

private struct MilkieFlexibleNumber: Decodable {
    let int64: Int64
